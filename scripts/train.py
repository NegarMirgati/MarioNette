...
def main(args):
    ...
    # NOTE: We need two separate dictioneries, one for player sprites, and one for others!
    ...
    trainer.add_callback(
        callbacks.VizCallback(suffix="", writer=writer, val_writer=None, frequency=2000)
    )
    trainer.add_callback(
        callbacks.VizCallback(
            suffix="_hard", writer=writer, val_writer=None, frequency=2000
        )
    )
    trainer.add_callback(
